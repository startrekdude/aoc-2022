...
static int evalpath(uint8_t *path, int n) {
...
void bruteforce6(int cut) {
	...
	path[5] = x6;
		...
		int meResult = evalpath(path, cut);
		int elResult = evalpath(path + cut, 6 - cut);
		int result = meResult + elResult;
		if (result > max) {
			printf("%d\n", result);
			max = result;
		}
...
void bruteforce15(uint8_t x1, int cut) {
	int max = 0;
	...
	path[14] = x15;
		...
		int meResult = evalpath(path, cut);
		int elResult = evalpath(path + cut, 15 - cut);
		int result = meResult + elResult;
		if (result > max) {
			printf("New max: %d -> %d\n", max, result);
			max = result;
		...
		i += 1;
		if (i % 100000000 == 0) {
			printf("Iterations: %llu/87178291200\n", i);
			double pct = ((double) i) / 87178291200.0;
			pct *= 100;
			printf("Percent complete: %lf\n", pct);
			time_t now = time(0);
			printf("Seconds elapsed: %lld\n", now - start);
		}
	}
	}
	}
...
int main(int argc, char **argv) {
	...
	int cut   = atoi(argv[2]);
	printf("Start: %d; Cut: %d\n", start, cut);
	bruteforce15(start, cut);
#endif
